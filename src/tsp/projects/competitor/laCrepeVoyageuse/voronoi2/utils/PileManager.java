...
import tsp.projects.competitor.laCrepeVoyageuse.voronoi2.fortune.BeachLine;
...
public class PileManager {
    ...
    public static int getAddIndexInPile(Point point, ArrayList<Point> pile) {
        ...
        while (!found && index < pile.size()) {
            ...
            else if (yPoint == yPile && xPoint == xPile && pile.get(index) instanceof GraphPoint) {
                found = true;
            }
    ...
    /**
     * Génère une pile de point trié à partir des zones de deux beachLines
     * On récupère les points qui composent la beachLine pour générer une pile de point
     * @param zones1 Zone de la 1ère beachline
     * @param zones2 Zone de la 2ème beachline
     * @param zones3 Zone de la 3ème beachline
     */
    public static ArrayList<Point> genPilePointWithZones(AbstractRotate rotate, ArrayList<Point> zones1, ArrayList<Point> zones2, ArrayList<Point> zones3) {
        ArrayList<Point> points = new ArrayList<>();
        ArrayList<Point> rotatePoints;
        points.addAll(zones1);
        points.addAll(zones2);

        rotatePoints = rotate.translatePoints(points);
        ArrayList<Point> sortPoints = PileManager.sortPoints(rotatePoints);

        if (sortPoints.size() > 1) {
            for (int i = (sortPoints.size() - 1); i >= 1 ; i--) {
                if (sortPoints.get(i).getId().equals(sortPoints.get(i - 1).getId())) {
                    sortPoints.remove(i);
                }
            }
        }
