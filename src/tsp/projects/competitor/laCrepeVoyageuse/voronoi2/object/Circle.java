...
public class Circle extends Point {
    ...
    public Circle(ArrayList<Point> points) throws Exception {
        this(points, generateIdWithValidation(points));
    }


    /**
     * Constructeur de la classe Circle avec un identifiant
     * @param points Liste des points qui composent le cercle (il faut obligatoirement que le cercle soit composé de 3 points)
     * @param id Identifiant du cercle
     */
    public Circle(ArrayList<Point> points, String id) throws Exception {
        if (points.size() != 3) {
    ...
    /**
     * Génère un identifiant à partir de 3 points (avec vérification)
     * @param points Liste de point
     * @return Identifiant au format String
     */
    private static String generateIdWithValidation(ArrayList<Point> points) throws Exception {
        if (points.size() != 3) {
            throw new Exception("Le nombre de points pour faire un cercle doit être égale à 3");
        }
        return generateId(points.get(0), points.get(1), points.get(2));
    }

    /**
     * Génère un identifiant à partir de 3 points
     * Les identifiants des points sont triés du plus petit au plus grand
    ...
    public static String generateId(Point point1, Point point2, Point point3) {
        int intId1 = Integer.parseInt(point1.getId());
        int intId2 = Integer.parseInt(point2.getId());
        int intId3 = Integer.parseInt(point3.getId());

        int id1 = Math.min(Math.min(intId1, intId2), intId3);
        int id3 = Math.max(Math.max(intId1, intId2), intId3);
        int id2 = (intId1 != id1 && intId1 != id3) ? intId1 : ((intId2 != id1 && intId2 != id3) ? intId2 : intId3);
