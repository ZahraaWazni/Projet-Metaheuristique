...
public class BeachLine {
    ...
    final double approximation = Math.pow(10, -15);

    // Instance de classe
    private static BeachLine instance = null;
     ...
     */
    public static BeachLine getInstance() {
        if (instance == null) {
            instance = new BeachLine();
        }
        return instance;
    }
     ...
     */
    public void updateBeach() throws Exception {
        ...
        if (zones.size() >= 2) {
            ...
            double xMinGraph = dataGraph.getMinPoint().getX();
            double xMaxGraph = dataGraph.getMaxPoint().getX();
            ...
            for (int i = 0; i < zones.size() - 1; i++) {
                ArrayList<Point> intersections = IntersectionManager.getIntersections(zones.get(i), zones.get(i + 1));
            ...
            for (int i = ranges.size() - 1; i >= 0; i--) {
                ...
                if ((xStartRange + approximation) > xEndRange) {
                    zones.remove(i);
                    ranges.remove(i);
                }
            }
